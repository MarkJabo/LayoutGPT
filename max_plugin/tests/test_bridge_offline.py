...
----------------------
Unit tests for the LayoutGPT bridge that run outside 3ds Max (no pymxs).
Covers tag_mapper, room_formatter, and the LayoutGPT response parser.
...
class TestDenormalizeTag:
    ...
    def test_fallback_to_category(self):
        # category not in LAYOUTGPT_TO_TAGS → returns category itself
...
class TestRoundtrip:
    """Verify that all TAG_TO_LAYOUTGPT keys survive normalize/denormalize."""
...
class TestRoomFormatter:
    """Use MockRoomInfo to avoid pymxs."""
    ...
    def test_room_px_length_longer_axis(self, formatter):
        # 5000 mm is longer; should map to 256 px
    ...
    def test_from_px_position(self, formatter):
        ...
        # Room origin at (0,0,0), scale = 256/5000
        scale = 5000 / 256
    ...
    def test_placement_in_bounds(self, formatter):
        in_bounds = {"pos_x": 2500.0, "pos_y": 2000.0}
        out_of_bounds = {"pos_x": 6000.0, "pos_y": 2000.0}
        assert formatter.placement_in_bounds(in_bounds)
        assert not formatter.placement_in_bounds(out_of_bounds)
...
class TestParse3DLine:
    ...
    def test_category_strips_digits(self):
        # LayoutGPT sometimes appends a digit to disambiguate
